...
public class MultiTypeAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    public MultiTypeAdapter(@NonNull List<?> items, int initialCapacity) {
        this(items, new ItemManager(initialCapacity));
    }
     ...
     */
    public MultiTypeAdapter(@NonNull List<?> items, @NonNull ItemViewPool pool) {
        this.items = items;
        this.typePool = pool;
    }
    ...
    public <T> void register(
        ...
        typePool.register(clazz, binder, new ItemViewLinker<T>() {
            @Override
     ...
     * @see #register(Class)
     */
    public void registerAll(@NonNull final ItemViewPool pool) {
     ...
     */
    public void setTypePool(@NonNull ItemViewPool typePool) {
        this.typePool = typePool;
    }

    @NonNull
    public ItemViewPool getTypePool() {
        return typePool;
    }
    ...
    int indexInTypesOf(@NonNull Object item) throws RuntimeException {
        int index = typePool.firstIndexOf(item.getClass());
        if (index != -1) {
            @SuppressWarnings("unchecked")
            ItemViewLinker linker = typePool.getLinkers().get(index);
            return index + linker.index(item);
        }
        throw new RuntimeException(String.format("Do you have registered the binder for %s.class in the adapter/pool?", item.getClass().getSimpleName()));
    }
    ...
    private void registerWithoutChecking(
            @NonNull Class clazz, @NonNull ItemViewBinder itemViewBinder, @NonNull ItemViewLinker linker) {
        checkAndRemoveAllTypesIfNeed(clazz);
